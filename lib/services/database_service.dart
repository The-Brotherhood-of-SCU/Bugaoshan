...
class DatabaseService {
  ...
  /// 一次性迁移余额历史的房间 key。
  ///
  /// 迁移和删除在同一事务中完成；目标已有相同记录时跳过插入，使得
  /// 偏好设置标记写入前即使进程中断，下一次重试也不会产生重复历史。
  Future<int> migrateBalanceRecords({
    required Map<String, String> roomKeyMappings,
    required Set<String> roomKeysToDelete,
  }) async {
    return _db.transaction((txn) async {
      var migrated = 0;
      for (final entry in roomKeyMappings.entries) {
        ...
        for (final row in rows) {
          final duplicate = existing.any(
            (candidate) =>
                candidate['balance_type'] == row['balance_type'] &&
          ...
          final copy = Map<String, dynamic>.from(row)
            ..remove('id')
            ..['room_key'] = newKey;
          await txn.insert('balance_records', copy);
          existing.add(copy);
          migrated++;
        }
        await txn.delete(
      ...
      for (final roomKey in roomKeysToDelete.difference(
        roomKeyMappings.keys.toSet(),
      )) {
        await txn.delete(
          'balance_records',
          where: 'room_key = ?',
      ...
      // 旧版 key 没有统一前缀。无法从绑定恢复归属的孤立记录也必须清理，
      // 否则它们会永久留在数据库中并在后续迁移中反复成为歧义数据。
      final legacyRows = await txn.query(
        'balance_records',
        columns: ['room_key'],
        distinct: true,
        where: 'room_key NOT LIKE ?',
        whereArgs: ['balance-v2:%'],
      );
